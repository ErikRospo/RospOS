...
In addition, Mini-8 had, in hindsight, _a very bad design_. Technically, it had 8 GP registers. But only `r0`-`r3` were actually usable as GP registers. `r4` was the RAM address register, and r5 was the RAM data register. Basically what this means is that when you want to read from RAM address X, you have to load X into `r4`, then read from `r5`. This isn't too bad, and you could even _technically_ use `r4` as a GP register if you were careful and didn't depend on `r5`. But it was still a very awkward design, and it made writing code more difficult than it needed to be. `r6` was just unused, which was a waste, plain and simple. `r7` was the program counter, which could be useful for certain things, but it also meant that you couldn't use `r7` as a GP register without horrendous consequences.
...
However, it didn't feel very _proper_, and I ended up writing my own custom binary format, `.rosp`, that contained the same information as the mmap.txt file, but that was contained in a single file and was more efficient to read from the VM.
...
- [lcc](https://drh.github.io/lcc/)
  - LCC is _old_. It was designed to be retargetable, but I found it to be difficult to understand and modify.
...
To do this, I had to implement a full parser for the C-like language, as well as a way to generate the assembly code from the parsed AST. The parser wasn't trivial to implement, as C has a classically ambiguous grammar that requires lookahead and some amount of "smarts" to parse correctly. I ended up using Lark's Earley parser, which is a general context-free parser that can handle ambiguous grammars, which made possible to implement the parser for C. As of right now, the compiler is a work-in progress, but it is _almost_ able to compile very basic programs.
...
### Foiled by a side project!

Around the time I was getting the compiler to a point where it could compile basic functions, I finally _actually_ tested the binaries it produced, but the fact that the VM was entirely textual and had absolutely no debugging tools beyond "step" and "run full blast" made it very difficult to figure out what was going wrong when the compiled code didn't work. To address this, I decided that it would be a good idea to implement a fully-fledged GUI and debugger for the VM, which would allow me to step through the code, inspect registers and memory, and generally have a much better experience when testing the compiled code.

Initially, I considered using a library called [Dear ImGui](https://github.com/ocornut/imgui) for the GUI. However, I quickly realized that it was not a good fit for my needs, as it was designed to be an immediate mode GUI library, which is not ideal for building a complex and interactive debugger interface. It had features that I didn't need and lacked features that I did need. Luckily, I figured that out before investing any time into it.

The other option I considered, and eventually went with, was using a library called [Qt](https://www.qt.io/), which is a very powerful and flexible GUI framework that is well-suited for building complex and interactive applications. It does have a learning curve, but I found it to be much more intuitive and easier to work with than Dear ImGui for my use case. Additionally, I had already used Qt's Python bindings for a different project, so I was already somewhat familiar with how it worked, which made it easier to get started.

I didn't end up actually writing much of the UI, as my vibe code addiction was in full swing at this point, so I just ended up pointing Copilot at the codebase and asking it to generate a basic UI for the debugger. It ended up being pretty good at generating the code for the UI, as it was able to understand the structure of the codebase and generate code that fit well with the existing code. The end result was a basic UI that allowed me to step through the code, inspect registers and memory, and generally have a much better experience when testing the compiled code. What I've learned (which should not be a surprise at all) is that Copilot is very good at generating code that follows well-established, common paradigms, but it struggles when it comes to generating code for more unique or specific use cases. It's also completely useless for debugging, as it has no real understanding of how code works or what the underlying issue might be, so it just ends up generating random code that may or may not be related to the issue at hand.

This is the first time I've really dug into building a C++ application, let alone a GUI C++ application, and it was definitely a learning experience. Signals and slots were pretty much the same between Python and C++, so that was one area that I didn't struggle with too much. But a lot of the tools and interdependencies that many C/C++ developers take for granted, such as build systems, header files, and similar, were all new to me, and it took a while to get used to how they all worked together. CMake made it easier to manage the build process and the integration with Qt, but it still took some time to figure out how to set everything up correctly.

### Binary V2

One of the main motivations for building the GUI debugger was to be able to, well, _debug_ the compiled code. At this point, the GUI was essentially just a glorified disassembler, as it allowed me to step through the code and see the assembly instructions being executed, but it was difficult to really understand how the original ROSC code was being translated into assembly, then into machine code, and then into the actual behavior of the program. To address this, I decided to implement a new feature in the assembler that would allow it to generate a mapping between the original ROSC code and the generated assembly code, which would then be used by the debugger to show the original ROSC code alongside the assembly code, and to allow me to step through the original ROSC code while still being able to see the corresponding assembly instructions and how they relate to the original code.

This was a bit of a complex feature to implement, as it required changes to both the assembler and the debugger. In the assembler, I had to implement a way to track the mapping between the original ROSC code and the generated assembly code, which involved keeping track of the line numbers and instruction offsets for both the ROSC code and the assembly code. In the debugger, I had to implement a way to display this mapping in the UI, which was not terribly difficult, but it did require some changes to how the code was displayed and how the stepping functionality worked.

In essence, the compiler now generates a sidecar file alongside the generated assembly code, which contains a mapping between the original ROSC code and the generated assembly code. When the assembler generates the machine code, it also consumes the sidecar file and generates a binary with a new segment. Recall that the original binary format had a simple header with a magic number, version, and segment number, followed by the actual segments of data (code, data, etc). The new format adds an additional segment that contains the mapping information, which is essentially a list of mappings between the original ROSC code and the generated assembly code. Each mapping contains the line number and instruction offset for both the ROSC code and the assembly code, which allows the debugger to correlate the two and display them side by side in the UI.

Note that in this case, the segments aren't real segments to be written into memory, so I needed a way to differentiate between debug segments and data segments. The solution was to introduce a new field in the segment header that indicates the type of the segment. At this point, I also took the opportunity to use another bit to introduce a "compressed" flag, which indicates whether the segment data is compressed or not. This allows me to compress the debug segment, which can be quite large, without having to worry about the data segments, which are usually much smaller and may not benefit as much from compression. The debug segment is compressed using gzip, which is a ubiquitous compression format that is widely supported and provides a good balance between compression ratio and speed. The assembler compresses the debug segment before writing it to the binary, and the VM decompresses it when loading the binary into memory, so there is no additional work required on the part of the user to take advantage of this feature.

Because the bitfield has separate bits for LOADABLE, DEBUG, and COMPRESSED, this means that both LOADABLE and DEBUG segments can be compressed. In general, though, the debug information tends to require more (and be able to be compressed more) than the actual code and data. Completely uncompressed with debug information, a test program generates a binary that is around 232KiB. With the debug information compressed, the same binary is around 12KiB, and with the data compressed, it lost 927 bytes. So the 20x reduction in size is clearly more important than the ~1KiB reduction, but it's nice to have the option to compress both if needed. That reduction may also change with a different program that has more data and larger empty spaces, as those tend to compress very well.

### Optimizations

A lot of these developments happened in parallel, so while I was working on, say, register allocation in the compiler (Believe me, there were many times), I'd spot a bit of code generated by the compiler that just seemed inefficient. For example, the compiler would generate code that looked like this:
...
Which is extremely inefficient, as it uses 3 instructions where 1 would work:

```
  ADDI r1, r1, 1    // binop + and assign str
```

It's even worse when considering that r5 and r6 may not have been free registers, which means that the compiler would have to generate additional instructions to save and restore those registers around this code, which would make it even more inefficient. This was a common pattern that I noticed in the generated code, and it was a clear indication that the compiler was not doing a good job of optimizing the generated assembly code. Some of the optimizations, like this one in particular, were implemented in the compiler, as it's an extremely common pattern to increment something by a small immediate value.

So, long story short, whenever I'm looking through the generated assembly code and I see a pattern of instructions that seems inefficient, I try to implement essentially a pattern-matching optimization in the compiler that looks for a given pattern of instructions and replaces it with a more efficient pattern. All of these optimizations are known as peephole optimizations, as they look at a small "peephole" of instructions and try to optimize them. This is a common technique used in compilers to optimize the generated code, and it most often leads to modest improvements in performance, as it eliminates unnecessary instructions and reduces the number of instructions executed. However, unlike more complex optimizations, it rarely leads to massive improvements in performance. As an example, with all optimizations running as the code exists as I write this, the overall number of removed instructions is 22.7%, which is a significant reduction, but that doesn't necessarily translate to a 22.7% improvement in performance, as it depends on which instructions were removed and how they were being used in the code. But still, even if it's not a 22.7% improvement in performance, it's still an improvement.

Other optimizations were implemented in the assembler, such as neutralizing `PUSH` and `POP` pseudo-instructions. When the compiler generates a call to a function, it generates a bunch of `PUSH` instructions to save the caller-saved registers, then it generates a `CALL` instruction, and then it generates a bunch of `POP` instructions to restore the caller-saved registers. However, if two functions are being called in a row, the `POP` instructions from the first function call and the `PUSH` instructions from the second function call can be neutralized, as they cancel each other out. For example, if we have the following code:
...
The `POP` instructions from the first function call and the `PUSH` instructions from the second function call can be neutralized, as they cancel each other out. The optimized code would look like this:
...
Of course, this is simplifying a bit, but the general idea is that oftentimes, there are sequences of `PUSH` and `POP` instructions that can be optimized by neutralizing them, which can lead to more efficient code. In this case, the registers must be the same for the `PUSH` and `POP` instructions to be neutralized, which is usually the case when the compiler generates code for function calls, as it uses a consistent set of caller-saved registers. This optimization was implemented in the assembler, as it is a low-level optimization that is specific to the instruction set and calling convention, and it is not something that the compiler would be aware of or able to optimize on its own.

Another optimization related to function calls and `PUSH`/`POP` instructions is related to the fact that functions always try to generate the most general, safest code, which may not be the fastest.

```
  LLI r10, 4294967295    // load immediate 4294967295
  PUSH r10    // stage reg arg 0
  PUSH r9    // stage reg arg 1
  POP r2    // load reg arg 1
  POP r1    // load reg arg 0
  CALL read_block
```

In this case, the function being called uses r1 and r2 as arguments, but r9 and r10 hold the arguments beforehand. The safe way to do this is using the stack to push all the arguments onto the stack then pop them in the right order to get the right values in the right registers, but this is inefficient, as each `PUSH` and `POP` instruction expands to 2 actual instructions. In this case, the optimizer can see that PUSH r9 followed immediately by POP r2 can be optimized to just `ADDI r2, r9, 0`, and that `PUSH r10` followed by `POP r1` can be optimized to `ADDI r1, r10, 0`, which would result in the following optimized code:

```
  LLI r10, 4294967295    // load immediate 4294967295
  ADDI r1, r10, 0    // load reg arg 0
  ADDI r2, r9, 0    // load reg arg 1
  CALL read_block
```

However, you may still see that we first load the immediate value into r10, and then we use `ADDI` to move it into r1, which is inefficient. The optimizer can see that r10 is not used for anything else, and that the value of r10 is only used to load the immediate value into r1, so it can optimize this code further by just loading the immediate value directly into r1, which would result in the following optimized code:

```
  LLI r1, 4294967295    // load immediate 4294967295 directly into r1
  ADDI r2, r9, 0    // load reg arg 1
  CALL read_block
```

These optimizations are each done in a pass after the initial code generation, where the optimizer looks for patterns in the generated assembly code that can be optimized, and then applies the appropriate transformations to optimize the code. For example, the first optimization looks for patterns of `PUSH` followed by `POP` instructions that can be neutralized, and the second optimization looks for patterns of `PUSH` followed by `POP` instructions that can be optimized to `ADDI` instructions, and then it looks for patterns of `LLI` followed by `ADDI` where the register loaded by the `LLI` is not used for anything else, which can be optimized to just loading the immediate value directly into the target register. These optimizations can lead to significantly more efficient code, as they eliminate unnecessary instructions and reduce the number of instructions executed, which directly translates to faster execution of the program.

The optimizations are applied iteratively, as optimizing one pattern may create new opportunities for optimization in the resulting code. In this example, we can only apply the `LLI` + `ADDI` optimization after we apply the PUSH/POP to ADDI optimization, as the former creates the pattern that the latter looks for. Each optimization pass is quick enough that applying them iteratively does not add significant overhead to the assembly process. After the first pass of optimizations, the majority of the low-hanging fruit is usually picked, and subsequent passes may yield diminishing returns, but they can still catch some additional optimizations that were not apparent in the initial code. In general, it's a good idea to apply these optimizations iteratively until no more optimizations can be found, as this ensures that the generated code is as efficient as possible given the patterns that the optimizer is looking for.

### The QT, the Minimal, and the Headless

At this point, the assembler and compiler were both in a good enough place that I didn't have to *constantly* be looking at the generated assembly code to understand what was going wrong when the compiled code didn't work. The GUI debugger was a huge help in this regard, as it allowed me to step through the code and see exactly what was happening at each step, which made it much easier to identify and fix issues in the compiler and assembler. However, all of my many users (still just me) would most likely want a way to *just run* the compiled code without having to go through the GUI debugger, as it is designed to be a debugger, not a runner. To address this, I implemented two new applications: a headless command-line application that can run the compiled code without any GUI, and a minimal version of the QT GUI that is designed to be a simple runner for the compiled code, without any of the debugging features. The headless application is useful for quickly running the compiled code without having to wait for the GUI to load, while the minimal QT application provides a more user-friendly way to run the compiled code without having to use the command line. 

All three applications (`rospovm_qt`, `rospovm_minimal`, and `rospovm_headless`) derive from the same core VM library that I wrote, which also helps cut down on compile time a bit. The core VM library contains all of the logic for loading the binary, executing the instructions, and managing the state of the VM, while the different applications provide different interfaces for interacting with the VM. The `rospovm_qt` application provides a full-featured GUI debugger interface, while the `rospovm_minimal` application provides a simple runner interface that just runs the compiled code and displaying the output, and the `rospovm_headless` application provides a command-line interface for running the compiled code without any output. 

Actually... That's not entirely true. One optimization I did was to have two different versions of the VM library. The full version, which is used by the `rospovm_qt` application, includes all of the debugging features and the ability to load the debug segment from the binary, which allows it to display the original ROSC code alongside the assembly code in the debugger. The minimal version, which is used by both the `rospovm_minimal` and `rospovm_headless` applications, does not include any of the debugging features and does not load the debug segment from the binary, which makes it much smaller and faster to run, as it doesn't have to deal with all of the additional data and logic related to debugging. This way, users who just want to run their compiled code without any debugging features can use the minimal version of the VM, while users who want to debug their code can use the full version of the VM with the QT GUI. Both are built from the same codebase, but the minimal version has directives to exclude the debugging features. 

Removing all that debug code from the minimal version of the VM library resulted in a significant speedup due to it not having to parse and load the debug segment or keep track of memory reads/writes. On my AMD Ryzen 7 8700G, the `_headless` version of the VM went from executing around 300k-400k instructions per second to around 600-700k instructions per second, which is a significant improvment.

For context, though, 600k instructions per second is not *that* fast. Modern CPUs (of which this is not one nor is it trying to be) are measured in billions of instructions per second. RospOS is currently around 8000 times slower than a modern CPU. *Extremely* roughly, this means that each virtual instruction takes 8000 real CPU cycles to execute (this is *very* wrong, but it gives a rough idea of the overhead of the VM). This is a significant overhead, but it's not unexpected given that this is a simple software implementation of a CPU, and it is not optimized for performance. There are many ways to optimize the VM to improve its performance, such as implementing a JIT compiler, optimizing the instruction decoding and execution logic, and similar techniques, but for now, I'm just focused on getting the basic functionality working and improving the performance incrementally over time.