from maps import register_map, i_to_r_map
...
def _emit_immediate_loading_for_label(label_name, rd):
    ...
    return [
        {"type": "i", "name": "addi", "rd": rd, "rs1": 0, "imm": {"label": label_name, "part": "high"}},
        {"type": "i", "name": "shli", "rd": rd, "rs1": rd, "imm": 16},
        {"type": "i", "name": "ori", "rd": rd, "rs1": rd, "imm": {"label": label_name, "part": "low"}},
    ]
    ...
    for instr in ast:
        ...
        if instr["type"] == "p":
            ...
            if name == "jmp":
                ...
                # JALR rd, TEMP_REG, 0
                out.append({"type": "j", "name": "jalr", "rd": rd, "rs1": TEMP_REG, "imm": 0})
        ...
        if instr["type"] in ["i", "l"] and isinstance(instr.get("imm"), dict) and instr["imm"].get("type") == "li":
            li = instr["imm"]
            ...
            if instr["type"] == "i":
                ...
                if rname is None:
                    raise AssertionError(f"Cannot convert I-op {instr['name']} to R-op for lifted constant handling")
                out.append({
                    "type": "r",
                    "name": rname,
                    "rd": instr["rd"],
                ...
                if instr["rs1"] == 0:
                    ...
                    out.append({"type": "l", "name": "lw", "rd": instr["rd"], "rs1": TEMP_REG, "imm": 0})
                else:
                    # Base + offset: compute TEMP_REG = TEMP_REG + rs1; then lw
                    out.append({"type": "r", "name": "add", "rd": TEMP_REG, "rs1": TEMP_REG, "rs2": instr["rs1"]})
                    out.append({"type": "l", "name": "lw", "rd": instr["rd"], "rs1": TEMP_REG, "imm": 0})
